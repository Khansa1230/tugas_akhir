...
<div class="page-heading">
    <section class="section">
        <div class="card">
            ...
            <form method="GET" action="{{ route('prediksi_per_mahasiswa_matakuliah_biologi_mahasiswa') }}" id="filter-form">
                @csrf
                <div class="card-body">
                    <!-- NIM dari login, readonly -->
                    <div class="form-group">
                        <label for="nim" class="text">NIM Mahasiswa:</label>
                        <input type="text" name="nim" id="nim" class="form-control text"
                    ...
                    <div class="form-group">
                        ...
                        <label for="predicted_lulus" class="mb-0 mr-3 text"> <!-- class="text" di sini -->
                            Training {{ $trainingValue }} dan Uji {{ $testingValue }}:
                        </label>

                        <select name="predicted_lulus" id="predicted_lulus" class="form-control text"> <!-- class="text" di sini -->
                            <option class="text" value="">Pilih Prediksi Lulus</option> <!-- class="text" di sini -->
                            @foreach(range(10, 100, 10) as $value)
                                <option class="text" value="{{ $value }}" {{ old('predicted_lulus', $predictedLulus) == $value ? 'selected' : '' }}> <!-- class="text" di sini -->
                                    {{ $value }}
                                </option>
                            @endforeach
                        </select>
                            ...
                            @if(!empty(request('nim')) && $results->isNotEmpty())
                                @foreach($results as $row)
                                    <tr>
                                        ...
                                        <td>{{ $row->kategori_seminar_proposal}}</td>
                                        <td>{{ $row->seminar }}</td>

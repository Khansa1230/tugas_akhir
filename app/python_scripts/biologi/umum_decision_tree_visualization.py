...
# Path File
# ==============================================
json_file_path = r'C:\xampp\htdocs\skripsi\app\python_scripts\biologi\data.json'
test_size_file_path = r'C:\xampp\htdocs\skripsi\app\python_scripts\biologi\test_size.json'

output_image_path = r'C:\xampp\htdocs\skripsi\public\images\biologi\tree.png'
output_image_relative = 'images/biologi/tree.png'  # untuk asset() Laravel
...
if not os.path.exists(json_file_path):
    ...
    try:
        ...
        def gain(df, col, target):
            ...
            gain_val = total_entropy - weighted_entropy
        ...
        def process(df, target, atribut, depth=0, max_depth=5, min_gain=0):
            ...
            for col in atribut:
                ...
                for index, row in df_dist.iterrows():
                    ...
                    html += f"<td>{row['Belum Lulus']}</td>"
                    html += f"<td>{row['Lulus']}</td>"
                    html += f"<td>{row['Entropy']:.4f}</td>"

                    if index == 0 and row['Gain'] != '':
                        html += f"<td>{row['Gain']:.4f}</td>"
                    else:
                        html += "<td></td>"
        ...
        target = 'kategori'
        atribut = [
            'jenis_sekolah', 'kategori_sks', 'kategori_ipk',
            'ipsmt_1', 'ipsmt_2', 'ipsmt_3', 'ipsmt_4',
            'ipsmt_5', 'ipsmt_6', 'ipsmt_7',
            'kategori_pkl', 'kategori_kkn',
            'kategori_seminar_proposal', 'kategori_seminar', 'judisium'
        ]

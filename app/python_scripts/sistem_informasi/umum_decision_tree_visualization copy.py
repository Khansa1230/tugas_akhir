...
# Path File
# ==============================================
json_file_path = r'C:\xampp\htdocs\skripsi\app\python_scripts\sistem_informasi\data.json'
test_size_file_path = r'C:\xampp\htdocs\skripsi\app\python_scripts\sistem_informasi\test_size.json'

output_image_path = r'C:\xampp\htdocs\skripsi\public\images\sistem_informasi\tree.png'
output_image_relative = 'images/sistem_informasi/tree.png'  # untuk asset() Laravel
...
if not os.path.exists(json_file_path):
    ...
    try:
        ...
        def gain(df, col, target):
            ...
            gain_val = round(total_entropy - weighted_entropy, 4)
        ...
        # ===== Fungsi untuk menampilkan data pangkas rapi =====
        ...
        def process(df, target, atribut, depth=0, max_depth=5, min_gain=0):
            ...
            html += f"<tr><td></td><td>{total}</td><td>{belum_lulus}</td><td>{lulus}</td><td>{ent}</td><td></td></tr>"
            ...
            for col in atribut:
                ...
                for index, row in df_dist.iterrows():
                    html += "<tr>"
                    html += f"<td>{row[col]}</td><td>{row['Total']}</td><td>{row['Belum Lulus']}</td><td>{row['Lulus']}</td><td>{row['Entropy']}</td>"
                    html += f"<td>{row['Gain']}</td>" if index == 0 else "<td></td>"
                    html += "</tr>"
            html += "</table>"

            # Simpan hasil HTML + distribusi (data pangkas)
            distribusi_str = f"=== Distribusi (subset depth={depth}) ===\n{data_pangkas(df)}"
            output["entropy_gain"].append({
                "html": html,
                "distribusi": distribusi_str
            })

            # Rekursi untuk tiap subset
            for val, subset in df.groupby(best_col):
                sub_output = process(subset, target, atribut, depth+1, max_depth, min_gain)
                for item in sub_output["entropy_gain"]:
                    if "distribusi" not in item or not item["distribusi"]:
                        item["distribusi"] = f"=== Distribusi (subset {best_col}={val}) ===\n{data_pangkas(subset)}"
                output["entropy_gain"].extend(sub_output["entropy_gain"])
        ...
        output['tree_text'] = export_text(
            model, feature_names=list(X.columns), decimals=3
        )
        ...
        # Ekstrak aturan
        all_paths = extract_paths_from_tree(model, list(X.columns), model.classes_)
        output['rules_lulus'] = [
